...
double **wav_in(FILE *wavfile, int32_t *channels, int32_t *samplecount, int32_t *samplerate)
{
	...
	*samplerate = tag[7];
	...
	{
		printf("Skipping encoder metadata section...\n");
		total = (((tag[1] - 20) - tag[4])) - 46; // Actual length for modern WAV files
		fseek(wavfile, (long)114, SEEK_SET);	 // Move to audio portion start
	}
	else
	{
		total = tag[12];
	...
	*samplecount = total / (bytes / 8) / *channels;
	...
	printf("Starting audio read at %li\n", ftell(wavfile));
	if (tag[10] == 8)
