...
void settingsinput(int32_t *bands, int32_t samplecount, int32_t *samplerate, double *basefreq, double maxfreq, double *pixpersec, double *bandsperoctave, int32_t Xsize, int32_t mode)
{
	...
	if (*basefreq != 0)
		set_min = 1; // count unset interdependant frequency-domain settings
	if (maxfreq != 0)
		set_max = 1;
	if (*bandsperoctave != 0)
		set_bpo = 1;
	...
	{
		if (*basefreq == 0)
			fread(basefreq, sizeof(double), 1, freqcfg); // load values from it if they haven't been set yet
		else
			fread(&trash, sizeof(double), 1, freqcfg);
		if (maxfreq == 0)
			fread(&maxfreq, sizeof(double), 1, freqcfg); // unless we have enough of them (unset==3)
		else
			fread(&trash, sizeof(double), 1, freqcfg);
		if (*bandsperoctave == 0)
			fread(bandsperoctave, sizeof(double), 1, freqcfg);
		else
			fread(&trash, sizeof(double), 1, freqcfg);
		if (*pixpersec == 0)
			fread(pixpersec, sizeof(double), 1, freqcfg);
		else
			fread(&trash, sizeof(double), 1, freqcfg);
	}
	else
	{
		if (*basefreq == 0)
			*basefreq = 200; // otherwise load default values
		if (maxfreq == 0)
			maxfreq = 23866.740;
		if (*bandsperoctave == 0)
			*bandsperoctave = 12;
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (argv[i][0] != '-') // if the argument is not a function
		{
			if (in_name == NULL) // if the input file name hasn't been filled in yet
				in_name = argv[i];
			else if (out_name == NULL) // if the input name has been filled but not the output name yet
				out_name = argv[i];
	...
	for (i = 0; i < strlen(in_name); i++)
		if (in_name[i] >= 'A' && in_name[i] <= 'Z')
			in_name[i] -= 'A' - 'a'; // make the string lowercase
	...
	if (mode == 2 || mode == 3)
	{
		...
		if (brightness != 1.0)
			brightness_control(image, Ysize, Xsize, brightness);
